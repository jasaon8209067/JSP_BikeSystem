...
<html>
    ...
    <body>
        ...
        <%  Class.forName("net.sourceforge.jtds.jdbc.Driver").newInstance();//載入驅動程式元件
            ...
            //刪除sql
            if ("Del".equals(op)) {
                sqlDel = "delete from Bike_Color where C_RecId =" + recId;
                stmt.executeUpdate(sqlDel);
            }

            //往上移動
                if ("Up".equals(op)) {
                    sqlUp = "select C_Sort from Bike_Color where C_RecId =" + recId;//找出現在的排序sort
                    rsSort = stmt.executeQuery(sqlUp);

                    if (rsSort.next()) {
                        nowSort = rsSort.getInt("C_Sort");
                        sqlUp = "SELECT TOP 1 C_Sort, C_RecId FROM Bike_Color WHERE C_Sort < " + nowSort + " order by C_Sort desc ";
                        ...
                        if (upSort != null && !upSort.equals("")) {
                            sqlUp = "UPDATE Bike_Color SET C_Sort = " + upSort + " WHERE C_RecId = " + recId;//自己和上面的交換位置
                            stmt.executeUpdate(sqlUp);
                            sql = "update Bike_Color set C_sort = " + nowSort + " where C_RecId = " + upRecId;//下面的交換成自己的位置
                ...
                //往下移動
                if ("Down".equals(op)) {
                    sqlDown = "select C_Sort from Bike_Color where C_RecId =" + recId;//找出現在的排序sort
                    rsSort = stmt.executeQuery(sqlDown);

                    if (rsSort.next()) {
                        nowSort = rsSort.getInt("C_Sort");
                        sqlDown = "SELECT TOP 1 C_Sort, C_RecId FROM Bike_Color WHERE C_Sort > " + nowSort + " order by C_Sort asc ";
                        ...
                        if (downSort != null && !downSort.equals("")) {
                            sqlDown = "UPDATE Bike_Color SET C_Sort = " + downSort + " WHERE C_RecId = " + recId;//自己和下面的交換位置
                            stmt.executeUpdate(sqlDown);
                            sql = "update Bike_Color set C_sort = " + nowSort + " where C_RecId = " + downRecId;//上面的交換成自己的位置
                            stmt.executeUpdate(sql);
                        }
                    }
                    rsSort.close();
                    rsSort = null;
                    response.sendRedirect("BikeColor_List.jsp");
                    return;
                }

            //關鍵字搜尋
            if (!keyword.equals("")) {
                sql = "select * from Bike_Color where (C_Name like '%" + keyword + "%' or C_Sort like '%" + keyword + "%') order by C_Sort";
            } else {
                sql = "select * from Bike_Color order by C_Sort";
            }
//            out.print(sql);
            ...
            selectPage += "</select>";
        %>
        <form action="BikeColor_List.jsp" method="post" id="B_Frm" name="B_Frm">

            <table border="0" width="100%">
                <tr>
                    <td><a href="BikeColor_List.jsp">列表</a>&nbsp;&nbsp;&nbsp;<a href="BikeColor_Edit.jsp">新增</a></td>
...
//                    while (rs.next()) {
            ...
            <tr>
                ...
                <td><a href="BikeColor_Edit.jsp?op=Edit&C_RecId=<%= rs.getString("C_recId")%>">編輯</a>&nbsp;&nbsp;&nbsp;
                    <a href="javascript:void(0)" onclick="Del('BikeColor_List.jsp?op=Del&C_RecId=<%=rs.getString("C_RecId")%>')">刪除</a>&nbsp;&nbsp;&nbsp;
                    <a href="BikeColor_List.jsp?op=Up&C_RecId=<%=rs.getString("C_RecId")%>">上</a>&nbsp;&nbsp;&nbsp;
                    <a href="BikeColor_List.jsp?op=Down&C_RecId=<%=rs.getString("C_RecId")%>">下</a>
                </td>
